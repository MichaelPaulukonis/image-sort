...
# refound @ https://web.archive.org/web/20170930074441/http://www.tiac.net/~sw/2008/10/Hilbert/hilbert.py
# converted to python3 via online converter (possibly erroneously)
# further added int() in a bunch of places when the interpreter bombed out complaining about floats.
# the output seems to be nice, so i didn't completely screw it up
...
def gray_encode_travel( start, end, mask, i ):
    ...
    return ( ( int(g) | int( g / modulus ) ) & mask ) ^ start

def gray_decode_travel( start, end, mask, g ):
    travel_bit = start ^ end
    modulus = mask + 1          # == 2**nBits
    rg = ( int(g) ^ start ) * ( modulus / ( travel_bit * 2 ) )
    return gray_decode( ( int(rg) | int( rg / modulus ) ) & mask )
